...
public class CacheData {
    ...
    /**
     * CacheData主要包含name,dataId,group,tenant,listeners,configFilterChainManager,
     * md5属性，所以可以猜测，一个CacheData主要对应的是一个配置集（配置文件），
     * 其中listeners这里主要使用ManagerListenerWrap，这个CacheData的内部类，listeners可以理解为客户端对该配置文件的监听者
     * 当配置更新后，Nacos会把配置最新的MD5值覆盖上一次的MD5值，同时当发现Listener的MD5值与CacheData本生MD5值不同使，则会触发配置更新的操作safeNotifyListener
     */
